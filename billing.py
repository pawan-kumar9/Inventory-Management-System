from os import fspath, name, stat
from tkinter import*
...
class BillClass:
    def __init__(self,root):
        ...
        self.product_Table.column("name", width=95)
        ...
        cTitle = Label(CustomerFrame, text="Customer Details", font=(
            "goudy old style", 15), bg="light grey").pack(side=TOP, fill=X)

        lbl_name = Label(CustomerFrame, text="Name", font=(
            "times new roman", 15, "bold"), bg="white").place(x=5, y=35)

        txt_name = Entry(CustomerFrame, textvariable=self.var_cname, font=("times new roman", 13), bg="light yellow").place(x=75, y=35, width=150)

        lbl_contact = Label(CustomerFrame, text="Contact", font=(
            "times new roman", 15, "bold"), bg="white").place(x=260, y=35)

        txt_contact = Entry(CustomerFrame, textvariable=self.var_contact, font=("times new roman", 13), bg="light yellow").place(x=340, y=35, width=140)
        ...
        btn_1 = Button(Cal_Frame, text='1', font=("arial", 15, 'bold'),
                       command=lambda: self.get_input(1), bd=5, width=4, pady=10, cursor="hand2").grid(row=3, column=0)
        btn_2 = Button(Cal_Frame, text='2', font=("arial", 15, 'bold'),
                       command=lambda: self.get_input(2), bd=5, width=4, pady=10, cursor="hand2").grid(row=3, column=1)
        btn_3 = Button(Cal_Frame, text='3', font=("arial", 15, 'bold'),
                       command=lambda: self.get_input(3), bd=5, width=4, pady=10, cursor="hand2").grid(row=3, column=2)
        btn_mul = Button(Cal_Frame, text='*', font=("arial", 15, 'bold'),
                         command=lambda: self.get_input('*'), bd=5, width=4, pady=10, cursor="hand2").grid(row=3, column=3)

        btn_0 = Button(Cal_Frame, text='0', font=("arial", 15, 'bold'),
                       command=lambda:self.get_input(0),bd=5, width=4, pady=15, cursor="hand2").grid(row=4, column=0)
        btn_c = Button(Cal_Frame, text='C', font=("arial", 15, 'bold'),
                       command=self.clear_cal,bd=5, width=4, pady=15, cursor="hand2").grid(row=4, column=1)
        btn_eq = Button(Cal_Frame, text='=', font=("arial", 15, 'bold'),
                       command=self.perform_cal,bd=5, width=4, pady=15, cursor="hand2").grid(row=4, column=2)
        btn_div = Button(Cal_Frame, text='/', font=("arial", 15, 'bold'),
                         command=lambda: self.get_input('/'), bd=5, width=4, pady=15, cursor="hand2").grid(row=4, column=3)
        ...
        cart_Frame.place(x=280, y=8, width=218, height=342)
        ...
        txt_p_name = Entry(Add_CartWidgetsFrame, textvariable=self.var_pname, font=("times new roman", 12), bg="light yellow",state='readonly').place(x=5, y=35,width=170,height=22)

        lbl_p_price = Label(Add_CartWidgetsFrame, text="Price per qty", font=("times new roman", 12), bg="white").place(x=210, y=5)

        txt_p_price = Entry(Add_CartWidgetsFrame, textvariable=self.var_price, font=(
            "times new roman", 12), bg="light yellow", state='readonly').place(x=210, y=35, width=150, height=22)

        lbl_p_qty = Label(Add_CartWidgetsFrame, text="Quantity", font=(
            "times new roman", 12), bg="white").place(x=390, y=5)

        txt_p_qty = Entry(Add_CartWidgetsFrame, textvariable=self.var_qty, font=(
            "times new roman", 12), bg="light yellow").place(x=390, y=35, width=100, height=22)

        self.lbl_inStock = Label(Add_CartWidgetsFrame, text="In stock", font=("times new roman", 12), bg="white")
        self.lbl_inStock.place(x=5, y=58)

        btn_clear_cart=Button(Add_CartWidgetsFrame,text="Clear",command=self.clear_cart,font=("times new roman",12,"bold"),bg="light grey",cursor="hand2").place(x=180,y=58,width=150,height=25)

        btn_add_cart=Button(Add_CartWidgetsFrame,text="Add / Update Cart",command=self.add_update_cart,font=("times new roman",12,"bold"),bg="orange",cursor="hand2").place(x=340,y=58,width=150,height=25)

        #=========billing area====================
        billFrame=Frame(self.root,bd=2,relief=RIDGE,bg="white")
        billFrame.place(x=922,y=110,width=350,height=410)

        bTitle = Label(billFrame, text="Customer Bill Area", font=(
            "goudy old style", 20, "bold"), bg="#ff0000", fg="white").pack(side=TOP, fill=X)
        ...
        btn_print.place(x=2,y=60,width=120,height=50)
        btn_clear_all=Button(billMenuFrame,text="Clear All",cursor="hand2",command=self.clear_all,font=("goudy old style",12,"bold"),bg="gray",fg="white")
        btn_clear_all.place(x=124,y=60,width=100,height=50)
        ...
        btn_generate.place(x=226, y=60, width=119, height=50)
...
#====================================All functions===============
    ...
    def add_update_cart(self):
        if self.var_pid.get()=='':
            ...
            messagebox.showerror("Error","Quantity is required",parent=
            self.root)
