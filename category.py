...
class categoryClass:
    def __init__(self, root):
        ...
        txt_name = Entry(self.root, textvariable=self.var_name,font=(
            "goudy old style", 18), bg="light yellow").place(x=50, y=170,width=300)

        btn_add = Button(self.root, text="ADD", command=self.add,font=(
            "goudy old style", 15), bg="#4caf50",fg="white",cursor="hand2").place(x=360, y=170, width=150,height=30)
        ...
        cat_frame.place(x=700, y=100, width=360, height=100)
        ...
        self.categoryTable.column("name", width=95)
        ...
        self.im1=self.im1.resize((475,250),Image.ANTIALIAS)
        ...
        self.im2 = self.im2.resize((475, 250), Image.ANTIALIAS)
        ...
        try:
            if self.var_name.get() == "":
                messagebox.showerror(
                    "Error", "Category name should be required", parent=self.root)
            else:
                ...
                if row != None:
                    messagebox.showerror(
                        "Error", "Category already present, try different", parent=self.root)
                else:
                    cur.execute(
    ...
    def delete(self):
        ...
        try:
            if self.var_cat_id.get() == "":
                ...
                if row == None:
                    messagebox.showerror(
                        "Error", "error, please try again", parent=self.root)
                else:
